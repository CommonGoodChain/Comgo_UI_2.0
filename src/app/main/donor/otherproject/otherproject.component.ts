 ...
 */
      this.otherProjectService.getUserDetails(body).then(res => {
  ...
  BCGetAllFundDetails() {
    ...
    if (routesPath[3] == 'otherproject') {
      ...
      */
      this.otherProjectService.getAllPublishedProjects().then(res => {
        console.log("getAllPublishedProjects response: ", res)
        this.project = res;
 ...
 */
      this.otherProjectService.getOrgPublishedProjects(sessionStorage.getItem('orgName')).then(res => {
        this.project = res;
        var otherProjectDetails = [];
        for (var i = 0; i < this.project.length; i++) {
          var otherSingleProject = {};
 ...
 * @description This function will return Published Projects of Organization
 */
      this.otherProjectService.getOrgPublishedProjects(this.routeParams).then(res => {
        this.project = res;
        var otherProjectDetails = [];
        for (var i = 0; i < this.project.length; i++) {
          var otherSingleProject = {};
 ...
 */
    this.otherProjectService.getAllSDG().then(res => {
      this.SDGData = res;
      ...
      if (this.userRules.donate_Project) {
        ...
        dialogRef.afterClosed().subscribe(result => {
          ...
          if (donationType == 'Donate') {
            ...
            */
            this.otherProjectService.getAllDonorListDB(data).then(res => {
              this.donorList = res;
